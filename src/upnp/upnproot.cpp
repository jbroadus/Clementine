...
DeviceRegistration::DeviceRegistration(const QString& descUrl, QObject* parent)
  : QObject(parent),
    descUrl_(descUrl.toUtf8()),
    registered_(false) {
}

bool DeviceRegistration::Register() {
  int ret = UpnpRegisterRootDevice2(UPNPREG_URL_DESC, descUrl_.constData(),
                                    0, 0,
                                    SCallback, this, &handle_);
...
int DeviceRegistration::SCallback(Upnp_EventType type, const void* event, void* cookie) {
...
int DeviceRegistration::Callback(Upnp_EventType type, const void* event) {
  ...
  case UPNP_CONTROL_ACTION_REQUEST:
    emit ActionRequest(Clementine::UpnpActionRequest(event));
    break;
  default:
...
void UpnpRoot::ActionRequest(const Clementine::UpnpActionRequest& req) {
...
bool UpnpRoot::Register() {
  ...
  connect(registration_.get(), SIGNAL(ActionRequest(Clementine::UpnpActionRequest)),
          SLOT(ActionRequest(Clementine::UpnpActionRequest)), Qt::BlockingQueuedConnection);
